...
export default async function OpengraphImage({ params }: { params: { orderId: string } }) {
  try {
    const result = await fetchOrder(params.orderId)
      ...
      (
        ...
        >
          ...
          <div style={{
            ...
            <span>Order:</span>
            <span style={{ fontFamily: 'ui-monospace, SFMono-Regular' }}>{params.orderId.slice(0, 10)}…{params.orderId.slice(-8)}</span>
          </div>
        </div>
      ),
      { ...size }
    ...
    return new ImageResponse(
      (
        <div style={{
          ...
          Failed to load order: {params.orderId}
        </div>
      ),
      { ...size }
    )
